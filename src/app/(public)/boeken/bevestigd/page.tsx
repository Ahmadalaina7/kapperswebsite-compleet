...
import { CheckCircle, Scissors, Mail, CalendarDays } from 'lucide-react';
...
function formatPrice(cents: string | undefined): string {
  if (!cents) return '';
...
export default async function ConfirmedPage({
  ...
  if (sp.service) rows.push({ label: 'Behandeling', value: sp.service });
  if (sp.date) {
    const formatted = formatDate(sp.date);
    rows.push({ label: 'Datum', value: formatted || sp.date });
  }
  if (sp.time) rows.push({ label: 'Tijdstip', value: sp.time });
  if (sp.price) {
    const p = formatPrice(sp.price);
